...
namespace QislEngine
{
    public class PigetScriptLibrary
    {
        ...
        public void Update()
        {
            Utils.Logs.Log("<!> ", $"{Name} -> Обновление манифеста");
            ...
            Utils.FileSystem.CreateFileWithText(Path.Combine(LibraryDirectory, ManifestFileName), JsonSerializer.Serialize(this));
            ...
            ScriptListAddresses.ForEach(scriptListAddress =>
            {
                ...
                pigetScripts.Scripts.ForEach(script => 
                {
                    ...
                    if (Directory.Exists(scriptDirectory))
                    {
                        ...
                        {
                            Utils.Logs.Log("<!> ", $"{Name} -> Обновление источников -> Скрипт {script.Name} не нуждается в обновлении");
                            return;
                        }
                        else
                        {
                    ...
                    Utils.FileSystem.CreateFileWithText(scriptManifestPath, JsonSerializer.Serialize(script));
                    Utils.FileSystem.CreateFileWithText(scriptBatExecutablePath, script.InitialScript);
                    Utils.Logs.Log("<!> ", $"{Name} -> Обновление источников -> Скрипт {script.Name} обновлен");
                });
            });
        }
